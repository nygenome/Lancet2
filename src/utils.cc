...
auto HammingDistWithin(std::string_view s1, std::string_view s2, usize max) -> bool {
  if (s1.length() != s2.length()) return false;

  usize differences = 0;
  for (usize idx = 0; idx < s1.length(); idx++) {
...
auto HasAlmostRepeatKmer(std::string_view seq, usize k, u32 max_mismatches) -> bool {
  const auto endOffset = seq.length() - k + 1;
  for (usize offset1 = 0; offset1 < endOffset; ++offset1) {
    for (usize offset2 = offset1 + k; offset2 < endOffset; ++offset2) {
      u32 numSeenMismatches = 0;

      for (usize idx = 0; idx < k; idx++) {
