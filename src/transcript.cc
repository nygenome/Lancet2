...
auto Transcript::VariantCov(SampleLabel label) const -> VariantHpCov {
  ...
  const auto altFwd = sampleCovs[0].GetNonZeroMinimum(Allele::ALT, Strand::FWD, isSomatic);
  const auto altRev = sampleCovs[0].GetNonZeroMinimum(Allele::ALT, Strand::REV, isSomatic);
