...
auto Transcript::VariantCov(SampleLabel label) const -> VariantHpCov {
  ...
  if (label == SampleLabel::TUMOR) {
    ...
    const auto altFwd =
        isSNV ? static_cast<u16>(std::round(sampleCovs[1].GetNonZeroMean(Allele::ALT, Strand::FWD, false)))
              : sampleCovs[1].GetNonZeroMinimum(Allele::ALT, Strand::FWD, false);
    const auto altRev =
        isSNV ? static_cast<u16>(std::round(sampleCovs[1].GetNonZeroMean(Allele::ALT, Strand::REV, false)))
