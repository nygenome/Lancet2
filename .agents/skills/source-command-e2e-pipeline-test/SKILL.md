---
name: "source-command-e2e-pipeline-test"
...
# source-command-e2e-pipeline-test

Use this skill when the user asks to run the migrated source command `e2e-pipeline-test`.
...
are configured in `.Codex/settings.local.json` (gitignored); copy
`.Codex/settings.local.json.example` and adjust if your checkout
location differs.
...
The full execution lives in `.Codex/scripts/e2e-pipeline-test.sh` and is
invoked here as a thin wrapper:

```bash
bash "$CLAUDE_PROJECT_DIR/.Codex/scripts/e2e-pipeline-test.sh"
```
...
`.Codex/scripts/e2e-pipeline-test.sh`. The `/audit-bundle` slash command
