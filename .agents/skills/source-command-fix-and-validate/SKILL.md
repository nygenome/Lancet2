---
name: "source-command-fix-and-validate"
...
# source-command-fix-and-validate

Use this skill when the user asks to run the migrated source command `fix-and-validate`.
...
if ! pixi run --quiet iwyu-fix; then
  ...
  rm -f .Codex/cache/validation-state.txt
  exit 1
fi
...
if [ "$all_ok" = "1" ]; then
  ...
  echo "$state" > .Codex/cache/validation-state.txt
  echo "✓ pre-commit gate marker updated (.Codex/cache/validation-state.txt)"
else
  rm -f .Codex/cache/validation-state.txt
