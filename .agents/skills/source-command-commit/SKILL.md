...
description: "Compose a Lancet2-conformant commit message from the staged diff. Reads .Codex/commit-style.json (grounded in .chglog/config.yml and docs_dev/style/cpp_style.md § Git commit messages) for rules, drafts subject and body in the project's two-section pattern, asks for confirmation, then commits."
---
...
Third, load the style rules. Read `.Codex/commit-style.json` to confirm the allowed types, the trivial-commit patterns, and the body-shape expectations. The rules there are derived directly from `.chglog/config.yml` and `docs_dev/style/cpp_style.md` § Git commit messages; treat them as authoritative.
