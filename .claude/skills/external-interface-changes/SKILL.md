...
- **No `tests/cli/` directory exists** — the CLI is exercised end-to-end via `.claude/scripts/e2e_pipeline_test.sh` and the broader `pixi run test` task. There are no unit tests for CLI parsing directly. If the CLI flag change has subtle parsing implications (e.g., a new alias, a default change), surface a recommendation to the user that an end-to-end test in `e2e_pipeline_test.sh` may need updating.
