...
The agent reads the cited frames in `src/lancet/`, identifies the root cause, and proposes the minimum fix. It has its own memory file at `.claude/agent-memory/sanitizer-expert.md` recording known-benign warnings and patterns of new warnings that turned out to be real bugs — consult that before reasoning from scratch.
