...
- Top-level `CMakeLists.txt` adds an `add_subdirectory(src/lancet/<new-layer>)` and updates the layer chain in `validate_layer_direction.py` (in the bundle's `.claude/hooks/`).
- The new library is added to `target_link_libraries` of any layer that depends on it.
- The new layer's path-scoped rule file: `.claude/rules/<new-layer>.md` (per the rules README).
...
If category 2 (new layer), the path-scoped rule file in `.claude/rules/<new-layer>.md` needs to be created. Use one of the existing rules (`base.md`, `hts.md`, etc.) as a template. The frontmatter must have:
...
The rule file goes in the same commit as the CMakeLists changes. The bundle's `validate_layer_direction.py` hook also needs updating to know about the new layer in the layer chain — that's a `.claude/hooks/` edit, which goes through the bundle's `/audit-bundle` review.
...
.claude/rules/assembly.md: new path-scoped rule.
