...
This section is for the calling session that just got findings from this agent. The findings are technical claims to verify, not orders to follow. The receiving session's discipline matters as much as the agent's reviewing rigor.

**Verify before implementing.** For each finding, before touching code, check it against the actual codebase. Read the cited file:line. Confirm the agent's claim. Most findings are correct, but the agent reads the diff cold and occasionally misses context that the calling session has — a callsite outside the diff window justifies a pattern; a previous PR's resolution makes a comment redundant; a benchmark constraint forces a non-obvious choice. Verify, don't assume.

**Don't performatively agree.** Phrases like "great point!", "you're absolutely right!", "thanks for catching that!" add no information and signal that the receiving session stopped thinking. State the technical conclusion or just fix the code. Actions speak louder than acknowledgments.

**Push back when the finding is wrong.** If the agent's reasoning misses context, push back with technical reasoning. Cite the specific evidence (file:line, benchmark number, PR number, layer rule). The agent expects this when warranted; it does not expect blind agreement. A pushback with evidence is a useful signal — it forces the agent (or the next reviewer) to engage with the actual constraint rather than the surface diff.

**Clarify before partial implementation.** If you understand 4 of 6 findings clearly but find 2 ambiguous, **stop**. Don't implement the 4 you understand and ask about the others later — the findings may interact, and partial understanding produces partially-wrong fixes. Ask for clarification on the ambiguous ones first, then implement all six together.

**Implement in severity order.** BLOCKER first (do-not-merge-without). MAJOR second (fix-before-merging). MINOR last (consider-in-follow-up). Within each tier: simple/mechanical fixes before complex restructures. Test each fix individually before stacking the next one — a regression in fix N+1 is hard to localize when fix N+1 is one of five concurrent changes.

**Surface NOLINT additions even if pre-existing.** If the agent flags a NOLINT addition that wasn't in the change author's summary of work, the disclosure gap is itself a finding. Add the disclosure to the change description; don't quietly let it slide. The disclosure rule (per the `clang-tidy-discipline` skill) exists so suppressions don't accumulate as quiet workarounds.

**When you correct your own pushback.** If you pushed back on a finding, then on closer reading determined the agent was correct, state the correction factually: "checked variant_call.cpp:142 — the agent was right, the lock isn't held across the loop. Fixing." Don't apologize at length or defend why you initially pushed back; the code itself shows the resolution.

**External vs internal findings.** Findings that surface broader issues (cross-file inconsistencies, accumulated NOLINT debt, layer-direction ambiguity, FORMAT field drift between `vcf_header_builder.cpp` and `variant_call.cpp`) are out-of-scope for this agent's specific diff-review charter. Set those aside as a separate follow-up — invoke the `doc-sync` skill in Mode B for documentation drift across a subdirectory, or open an `/audit-bundle` ticket for bundle-level issues. Don't conflate "this diff" with "the codebase needs work."

The fresh-reviewer agent's value is the distance it has from the change. Receiving the findings well — verify, push back when warranted, implement in severity order, clarify before partial action — is what turns that distance into shipped quality.
