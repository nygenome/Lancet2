...
- **`pre_commit_gate.sh`** — verifies that `/check` has passed against the EXACT working-tree state about to be committed. Does NOT re-run validation. Compares `git stash create` (an atomic commit-object hash representing the full working-tree state) against a marker at `.claude/cache/last-check-state` written by `/check` on full success. Match → silent exit 0 (~50ms). Mismatch or missing marker → exit 2 with remediation pointing at `/check`. Skips silently when the staged diff has no entries matching C++ source, CMake files, build-helper scripts (`scripts/*.{py,sh}`), or `.clang-tidy`/`.clang-format` — those commits don't need `/check` and don't trigger the marker check. The actual heavy validation (Release build + lint + IWYU + test, 3-7 min warm) runs only inside `/check`, on demand; the gate itself is hash-comparison fast.
...
- **`stop_reminder.sh`** — Stop-event hook that prints one line if the working tree has unstaged/staged C++/CMake changes (and is silent otherwise). Replaces the previous heavy `stop_validate.sh` which ran a 5–15 min build/test/lint gauntlet after every assistant turn. The Stop event fires after every turn, not just at session end; running heavy validation there made the implicit cost too high. The reminder version is ~50ms regardless of state.
...
Every Lancet2 hook is block-or-pass. Earlier iterations of the bundle had a soft-warning class (exit 0 with stderr text noting a heuristic concern) that Claude was expected to acknowledge in its next turn. That mechanism produced too much wallpaper output for too little signal — soft-warnings on commits that passed anyway, soft-warnings on stylistic preferences with real edge cases, soft-warnings that became invisible because they fired too often. The current design emits stderr only when the hook is blocking. If a check would fire as a heuristic, it doesn't belong in a hook; it belongs in a skill or in `AGENTS.md` where Claude can apply judgment.
...
The eight break down as: three deterministic source-edit enforcement (`block_protected_paths`, `validate_layer_direction`, `validate_naming`), one Bash-safety enforcement (`block_dangerous_bash`), three commit-time gates (`validate_commit_message`, `pre_commit_summary`, `pre_commit_gate`), and one Stop-time reminder (`stop_reminder`).
