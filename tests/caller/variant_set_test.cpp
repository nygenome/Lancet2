...
TEST_CASE("Topological VariantExtractor securely populates Multiallelic Payload Sets", "[lancet][caller][VariantSet]") {
    
    // We bind a linear Smith-Waterman engine to dynamically forge graphs universally natively out of strings.
    // The graph topology will authentically mimic organic topological sequencing branches.
    auto alignment_engine = spoa::AlignmentEngine::Create(spoa::AlignmentType::kNW, 3, -5, -3);

    SECTION("Extracts isolated Biallelic SNV dynamically managing VCF alignment anchoring universally") {
        // [REF]:  A T C G 
        // [ALT]:  A G C G   => (SNV: T -> G)
        
        spoa::Graph graph{};
        std::vector<std::string> seqs = {"ATCG", "AGCG"};
        for (const auto& s : seqs) graph.AddAlignment(alignment_engine->Align(s, graph), s);

        VariantSet caller_set; 
        caller_set.ExtractVariantsFromGraph(graph, core::Window{}, 100);

        // Verification Check: Our sweep relies on universal matching. A match generated 'AT'/'AG' originally
        // natively encompassing the anchor bounds identically. Then, the left-truncation algorithm strips
        // away universally useless left-alignment bases because no element resulted in a critical empty string 
        // violating bounds rules! 
        ...
        REQUIRE(var.mGenomeChromPos1 == 101); // Biologically validated string coordinate slid 1 index to the right 
    }
    ...
    SECTION("Encapsulates massively overlapping varying mutations tracking natively identically to Haplotypes") {
        ...
        //
        // Our Synchronous-Sink sweeps ALL paths identically concurrently natively. Because ALT1 and ALT2
        // topologically branch entirely off mathematically contiguous identical 'A' Anchor Nodes, they are  
        // fundamentally synthesized together merging continuously until they natively intuitively rendezvous!
        // Downstream SNVs act perfectly independent from anterior topological structures.
        ...
        // The Topological Sink now perfectly recognizes that the TGT Deletion mathematically spans completely
        // adjacent to the G->A SNV! This seamlessly fuses ALL of them natively into ONE massive multi-allelic block!
        REQUIRE(caller_set.Count() == 1);
        ...
        // We will just dynamically check presence to be algorithmically safe against graph-internal topological sorting behaviors!
        bool found_del = false;
        bool found_c_snv = false;
        bool found_a_snv = false;
        
        std::ranges::for_each(first_var.mAlts, [&](const auto& alt) {
    ...
    SECTION("Calculates a Pure Structural Insertion gracefully bounded universally against the exact Reference Target") {
        //                                  [ALT]
        //                          .--> (A)[3] --> (A)[4] --.
        //                         /                          \
        ...
        REQUIRE(caller_set.Count() == 1);
        auto var = *caller_set.begin();
        REQUIRE(var.mRefAllele == "T"); // Universal VCF Left-Alignment rule binds floating inserts correctly!
        REQUIRE(var.mAlts.size() == 1);
    ...
    SECTION("Safeguards symmetrical boundaries against MNP shattering properly natively") {
        ...
        REQUIRE(caller_set.Count() == 1);
        auto var = *caller_set.begin();
        // Since VCF normalizers strip rightward and leftward independently, if the internal string core differs, 
        // they cannot strip identically. MNP's logically hold their total structure identically natively!
    ...
    SECTION("Fuses massive Complex Variants natively without misalignment biases during Extraction") {
        ...
        // SPOA logically optimizes this topological structure! Instead of a massive monolithic mismatch, 
        // the engine perfectly decoupled them into two isolated dynamic events natively:
    ...
    SECTION("Handles Nested N-Way Sink Synchronization effectively inside dense mutational clusters") {
        //                          .--> (G)[3] --.     [ALT1: SNV]
        //                         /               \
        //                        +----> (A)[4] ----+   [ALT2: SNV]
        //                       /                   \
        ...
        std::ranges::for_each(var.mAlts, [&](const auto& alt) {
            if (alt.mSequence == "AG") found_g = true;
            if (alt.mSequence == "AA") found_a = true;
            if (alt.mSequence == "A") found_del = true; // Deletions strictly hold the universal 'A' Anchor!
        });
