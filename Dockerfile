...
RUN DEBIAN_FRONTEND="noninteractive" apt-get update && apt-get --yes --no-install-recommends upgrade && \
...
RUN DEBIAN_FRONTEND="noninteractive" git clone https://github.com/nygenome/Lancet2.git && cd /Lancet2 && mkdir build && \
    cd build && cmake -GNinja -DCMAKE_BUILD_TYPE=Release -DLANCET_BUILD_ARCH="skylake-avx512" .. && ninja -v
