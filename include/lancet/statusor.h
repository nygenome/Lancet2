...
class StatusOr : private internal_statusor::StatusOrData<T>,
 ...
 public:
  using element_type = T;  // DEPRECATED: use `value_type`.
  using value_type = T;
  ...
  auto operator=(const StatusOr&) -> StatusOr& = default;

  // StatusOr<T> will be move constructible/assignable if T is move
  // constructible.
  StatusOr(StatusOr&&) noexcept = default;
  auto operator=(StatusOr&&) noexcept -> StatusOr& = default;
  ...
  template <typename U, typename std::enable_if<std::is_convertible<U, T>::value>::type* = nullptr>
  auto operator=(const StatusOr<U>& other) -> StatusOr&;
  template <typename U, typename std::enable_if<std::is_convertible<U, T>::value>::type* = nullptr>
  auto operator=(StatusOr<U>&& other) -> StatusOr&;
  ...
  auto operator=(const absl::Status& status) -> StatusOr&;
  ...
  auto operator=(absl::Status&& status) -> StatusOr&;

  // Returns this->status().ok()
  [[nodiscard]] auto ok() const -> bool { return this->status_.ok(); }

  // Returns a reference to our status. If this contains a T, then
  // returns Status::OK().
  [[nodiscard]] auto status() const& -> const absl::Status&;
  ...
  auto ConsumeValueOrDie() -> T { return std::move(ValueOrDie()); }
...
template <typename T>
auto StatusOr<T>::operator=(const absl::Status& status) -> StatusOr<T>& {
  this->Assign(status);
  return *this;
}
...
template <typename T>
auto StatusOr<T>::operator=(absl::Status&& status) -> StatusOr<T>& {
...
inline StatusOr<T>::StatusOr(const StatusOr<U>& other) : Base(static_cast<const typename StatusOr<U>::Base&>(other)) {}

template <typename T>
template <typename U, typename std::enable_if<std::is_convertible<U, T>::value>::type*>
inline auto StatusOr<T>::operator=(const StatusOr<U>& other) -> StatusOr<T>& {
  if (other.ok()) {
...
template <typename T>
template <typename U, typename std::enable_if<std::is_convertible<U, T>::value>::type*>
inline StatusOr<T>::StatusOr(StatusOr<U>&& other) : Base(static_cast<typename StatusOr<U>::Base&&>(other)) {}

template <typename T>
template <typename U, typename std::enable_if<std::is_convertible<U, T>::value>::type*>
inline auto StatusOr<T>::operator=(StatusOr<U>&& other) -> StatusOr<T>& {
  if (other.ok()) {
...
template <typename T>
auto StatusOr<T>::status() const& -> const absl::Status& {
  return this->status_;
}
template <typename T>
auto StatusOr<T>::status() && -> absl::Status {
...
template <typename T>
auto StatusOr<T>::ValueOrDie() const& -> const T& {
  this->EnsureOk();
  return this->data_;
}

template <typename T>
auto StatusOr<T>::ValueOrDie() & -> T& {
  this->EnsureOk();
  return this->data_;
}
...
template <typename T>
auto StatusOr<T>::operator->() const -> const T* {
  this->EnsureOk();
  return &this->data_;
}

template <typename T>
auto StatusOr<T>::operator->() -> T* {
  this->EnsureOk();
  return &this->data_;
}
