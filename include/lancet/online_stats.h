...
class OnlineStats {
 public:
  ...
  void Add(const double sample) {
    ...
    const auto normalizedDelta = delta / num;

    moment1 += normalizedDelta;
    moment2 += (delta * normalizedDelta * oldNum);
  }
  ...
  auto Merge(const OnlineStats& other) -> OnlineStats& {
    ...
    moment1 = ((num * moment1) + (other.num * other.moment1)) / static_cast<double>(newNum);
    moment2 = moment2 + other.moment2 + delta2 * num * other.num / static_cast<double>(newNum);
    num = newNum;
